...
%s
    ...
    def test_write(self):
        ...
        base_cmd = ("cassandra-stress write cl=QUORUM duration=60m "
                    "-schema 'replication(factor=3)' -port jmx=6868 "
                    #"-log interval=60 "
                    "-mode cql3 native -rate threads=250 -errors ignore "
                    "-pop seq=1..10000000")
    ...
    def test_read(self):
        ...
        """
        base_cmd_w = ("cassandra-stress write cl=QUORUM n=30000000 "
                      "-schema 'replication(factor=3)' -port jmx=6868 "
                      "-log interval=60 "
                      "-mode cql3 native -rate threads=500 -errors ignore "
                      "-pop seq=1..30000000")
        base_cmd_r = ("cassandra-stress read cl=QUORUM duration=50m "
                      "-schema 'replication(factor=3)' -port jmx=6868 "
                      "-log interval=60 "
                      "-mode cql3 native -rate threads=500 -errors ignore "
