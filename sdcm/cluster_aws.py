...
class ScyllaAWSCluster(cluster.BaseScyllaCluster, AWSCluster):
    ...
    def node_setup(self, node, verbose=False, timeout=3600):
        ...
        if not cluster.Setup.REUSE_CLUSTER:
            ...
            node.remoter.run('sudo sed -ie "s/--default-log-level info/--default-log-level info --logger-log-level kmip=debug:replicated_key_provider=debug /g" /etc/sysconfig/scylla-server')
