...
class BaseNode(AutoSshContainerMixin, WebDriverContainerMixin):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def systemctl(self):
        self.wrap_cmd_with_permission('systemctl')

    def journalctl(self):
        self.wrap_cmd_with_permission('journalctl')
    ...
    def stop_scylla_server(self, verify_up=False, verify_down=True, timeout=300, ignore_status=False):
        ...
        if self.is_ubuntu14():
            ...
            self.remoter.run(f'{self.systemctl} stop scylla-server.service', timeout=timeout, ignore_status=ignore_status)
        if verify_down:
            self.wait_db_down(timeout=timeout)
    ...
    def node_setup(self, node: BaseNode, verbose: bool = False, timeout: int = 3600, wait_db_up: bool = True):  # pylint: disable=too-many-branches
        ...
        if not Setup.REUSE_CLUSTER:
            ...
            if node.is_nonroot_install:
                ...
                node.remoter.run('./install_root/sbin/scylla_setup --no-raid-setup --no-io-setup', ignore_status=True)
                node.remoter.send_files(src='./configurations/io.conf', dst='./install_root/etc/scylla.d/')
                node.remoter.send_files(src='./configurations/io_properties.yaml', dst='./install_root/etc/scylla.d/')
