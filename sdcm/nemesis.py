...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def disrupt_corrupt_then_scrub(self):
        ...
        """
        # Create test ks and cf before refresh sstables
        node = self.target_node
        with self.cluster.cql_connection_patient(node) as session:
            session.execute("""
                ...
                """)
            session.execute("""
                CREATE TABLE IF NOT EXISTS ks.cf (
                    pk text,
                    ck int,
                    s int,
                    v int,
        ...
        # Copy invalid sstables to upload directory of target node, and load the sstables by refresh
        result = node.remoter.sudo("ls -t /var/lib/scylla/data/ks/")
        cf_dir = result.stdout.split()[0]
        upload_dir = os.path.join('/var/lib/scylla/data/ks/', cf_dir, 'upload')
        node.remoter.send_files(src='data_dir/sstable_with_invalid_fragment', dst='/tmp')
        node.remoter.sudo(f'mv /tmp/sstable_with_invalid_fragment/ks/cf-test/* {upload_dir}')
        node.remoter.sudo(f'sudo chown -R scylla:scylla {upload_dir}')
        node.run_nodetool(sub_cmd="refresh", args="-- ks cf")

        self.log.debug("Rebuild sstable file by scrub, scrub will be stop for invalid sstable.")
        node.run_nodetool("scrub", args="ks cf")

        self.log.debug("Rebuild sstable file by scrub with `--skip-corrupted`, corrupted data file will be skipped.")
        with ignore_scrub_invalid_errors():
            node.run_nodetool("scrub", args="--skip-corrupted ks cf")
